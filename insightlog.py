...
def get_requests(service, data=None, filepath=None, filters=None):
    ...
    if filters:
        # apply_filters will now raise exception if file is missing
        filtered_data = apply_filters(filters, data=data, filepath=filepath)
    else:
        if filepath:
            # Fix: Removed try-except, let it raise FileNotFoundError
            with open(filepath, 'r', encoding='utf-8', errors='replace') as f:
                filtered_data = f.read()
        else:
            filtered_data = data
    
    # Standardize return: Empty list if no data, instead of None
